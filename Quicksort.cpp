...
__attribute__((noinline)) void quickSort(long* lst, unsigned long len)
{
    ...
    asm __volatile__(
        ...
        //"pushq %7\n\t"
        "movq %7, %3\n\t"           //Since we know we need to recurse, we can just immediately load these bounds.
        "jmp recurse\n\t"
        "recurse_or_exit:\n\t"
...
int main(void)
{
    ...
    double stdSortTime = 0;
    std::clock_t start;
    std::cout << "Array Size: " << myData_len << std::endl << "Number of Trials: " << num_trials << std::endl;
    std::cout << "Starting trials..." << std::endl;

    for(unsigned int i=0; i<num_trials; i++)
    {
        shuffle(myData, myData_len);
        long* stdData = copy_list(myData, myData_len);
        ...
        myQSortTime += (std::clock() - start)/(double)CLOCKS_PER_SEC;
        if(!isSorted(myData, myData_len)) {
            std::cout << "List not properly sorted!!" << std::endl;
            for(unsigned long i=0; i<myData_len; i++)
            {
                std::cout << myData[i] << ",";
            }
        ...
        //std::cout << "Done sorting." << std::endl << "Time taken (s): " << dur << std::endl;

        //std::cout << std::endl << "std::sort: ";
        start = std::clock();
        std::sort(stdData, stdData+myData_len);
        stdSortTime += (std::clock() - start)/(double)CLOCKS_PER_SEC;
        //std::cout << "Done sorting." << std::endl << "Time taken (s): " << dur << std::endl << std::endl << std::endl;
        ...
        std::cout << std::endl;*/
        //delete[] stdData;
        //std::cout << "One trial completed." << std::endl;
    }

    std::cout << "QSort avg time (s): " << myQSortTime/num_trials << std::endl;
    std::cout << "std::sort avg time(s): " << stdSortTime/num_trials << std::endl;
...
long* copy_list(long* lst, const unsigned long& len)
{
    long* the_copy = new long[len];
    for(unsigned long i = 0; i<len; i++)
    {
        the_copy[i] = lst[i];
    }
...
bool isSorted(long* lst, const unsigned long& len)
{
    long init=*lst;
